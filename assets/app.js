...
- =========================== */
  ...
  let currentCarouselIndex = 0;  // ← индекс карусели
...
- TRANSFER UI (upload/download progress)
- =========================== */
  let _xferToast = null;
...
wrap.id = “xferToast”;
wrap.style.cssText = `position:fixed; left:12px; right:12px; bottom:12px; z-index:99999; background:rgba(17,19,23,0.92); color:#fff; border-radius:14px; padding:12px; box-shadow:0 10px 30px rgba(0,0,0,0.25); backdrop-filter:blur(8px); display:none;`;

wrap.innerHTML = `<div style="display:flex;align-items:center;gap:10px;"> <div id="xferSpin" aria-hidden="true" style="width:18px;height:18px;border-radius:50%; border:2px solid rgba(255,255,255,0.25); border-top-color:#fff; animation:xferSpin 0.9s linear infinite;"></div> <div style="flex:1;min-width:0;"> <div id="xferTitle" style="font-weight:700;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Передача…</div> <div id="xferSub" style="margin-top:2px;font-size:12px;opacity:0.9;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">—</div> </div> <button id="xferHideBtn" style="appearance:none;border:0;background:rgba(255,255,255,0.14);color:#fff; border-radius:10px;padding:6px 10px;font-size:12px;cursor:pointer;">Скрыть</button> </div> <div style="margin-top:10px;"> <div style="height:8px;background:rgba(255,255,255,0.18);border-radius:999px;overflow:hidden;"> <div id="xferBar" style="height:100%;width:0%;background:#fff;border-radius:999px;"></div> </div> <div id="xferPct" style="margin-top:6px;font-size:12px;opacity:0.9;">—</div> </div>`;
...
wrap.querySelector(”#xferHideBtn”).addEventListener(“click”, ()=>{ wrap.style.display=“none”; });
_xferToast = wrap;
return _xferToast;
}
...
if(elBar){ const pseudo = clamp((loaded / (20*1024*1024))*100, 5, 95); elBar.style.width = pseudo.toFixed(0)+”%”; }
...
if(autoHideMs > 0){ setTimeout(()=>{ if(wrap) wrap.style.display=“none”; }, autoHideMs); }
}
...
- =========================== */
  ...
  try{ onUploadProgress({ loaded:Number(e.loaded||0), total:e.lengthComputable?Number(e.total||0):null }); }catch{}
  };
  }
  if(typeof onDownloadProgress === “function”){
  xhr.onprogress = (e)=>{
  try{ onDownloadProgress({ loaded:Number(e.loaded||0), total:e.lengthComputable?Number(e.total||0):null }); }catch{}
  };
  }
  ...
  resolve({ ok:status>=200&&status<300, status, statusText:xhr.statusText||””, responseText:text, response:xhr.response, getHeader:(name)=>{ try{ return xhr.getResponseHeader(name); }catch{ return null; } } });
  };
...
- =========================== */
  ...
  method,
  headers: { …(json ? { “Content-Type”:“application/json” } : {}), …authHeaders(), …headers },
...
- =========================== */
  ...
  throw new Error(`Файл слишком большой (лимит ${fmtBytes(WORKER_UPLOAD_LIMIT_BYTES)}).`);
  }
  ...
  showXfer({ title:“Загрузка в облако”, sub:`${file.name||"file"} • ${fmtBytes(file.size||0)}`, determinate:true });
  const res = await xhrRequest({ url, method:“PUT”, headers, body:file, responseType:””,
  onUploadProgress:({ loaded, total })=>{ updateXfer({ loaded, total:total||(file.size||null), title:“Загрузка в облако”, sub:`${file.name||"file"}` }); }
  });
...
if(!resp.body || typeof resp.body.getReader !== “function”) return await resp.blob();
let total = null;
try{ const cl = resp.headers.get(“content-length”); if(cl) total = Number(cl)||null; }catch{}
showXfer({ title:“Скачиваю из облака”, sub:”…”, determinate:!!(total&&total>0) });
...
if(done) break;
if(value){ chunks.push(value); loaded += value.byteLength||value.length||0; updateXfer({ loaded, total, title:“Скачиваю из облака”, sub:”” }); }
}
...
- BIG FILE direct upload
- =========================== */
  async function directUploadLargeFileToR2({ itemId, puchokId, file }){
  if(!file) throw new Error(“NO_FILE”);
  let presign;
  try{
  presign = await apiJson(r2PresignPath(), { method:“POST”, json:{ itemId, item_id:itemId, puchok_id:puchokId, name:(file.name||“file”).toString(), mime:(file.type||“application/octet-stream”).toString(), size:Number(file.size||0), source:“add_file” } });
  }catch(e){
  throw new Error(`Обходная загрузка не настроена.\nНужен POST ${r2PresignPath()}.\n${(e?.message||e)}`);
  }
  ...
  if(!uploadUrl) throw new Error(“Воркер вернул presign без uploadUrl.”);
  showXfer({ title:“Загрузка (direct)”, sub:`${file.name||"file"} • ${fmtBytes(file.size||0)}`, determinate:true });
  const upRes = await xhrRequest({ url:uploadUrl, method, headers:{ …extraHeaders, …(extraHeaders[“Content-Type”]?{}:{“Content-Type”:(file.type||“application/octet-stream”)}) }, body:file, responseType:””,
  onUploadProgress:({ loaded, total })=>{ updateXfer({ loaded, total:total||(file.size||null), title:“Загрузка (direct)”, sub:`${file.name||"file"}` }); }
  });
  if(!upRes.ok){
  ...
  throw new Error(`Direct upload не прошёл: HTTP ${upRes.status} ${txt||""}`.trim());
  }
  finishXfer({ ok:true, title:“Загружено”, sub:“Файл в R2. Финализирую…”, autoHideMs:0 });
  const done = await apiJson(itemBlobCompletePath(itemId), { method:“POST”, json:{ key, name:(file.name||“file”).toString(), mime:(file.type||“application/octet-stream”).toString(), size:Number(file.size||0), source:“add_file” } });
...
- =========================== */
  ...
  return { id:row.id, title:row.title, createdAt:row.created_at, updatedAt:row.updated_at, entries:[], items:[], audioRowId:null };
  }
  function mapRowRow(row){
  return { id:row.id, puchokId:row.puchok_id, type:row.type, title:row.title||””, createdAt:row.created_at, updatedAt:row.updated_at };
  }
  ...
  const subTitle = e.title || e.subpuchok_title || e.sub_title || (e.subpuchok && e.subpuchok.title) || (e.puchok && e.puchok.title) || null;
  const rowType = e.row_type || e.type || (e.row && e.row.type) || null;
  const rowTitle = e.row_title || e.title || (e.row && e.row.title) || null;
  return { id:e.id||uid(), kind, refId, orderIndex, subTitle, rowType, rowTitle };
  }
  function mapItemRow(row){
  const meta = parseMeta(row.meta) || null;
  const it = { id:row.id, type:row.type, title:row.title||null, content:row.content||null, url:row.url||null, mime:row.mime||null, size:Number(row.size||0), meta:meta||null, createdAt:row.created_at, updatedAt:row.updated_at };
  ...
  if(meta._rowId) it._rowId = meta._rowId;
  }
  if(it.type === “file” && it.mime && it.mime.startsWith(“image/”)) it.type = “image”;
  return it;
  }
...
db.rows[row.id] = { row, items, updatedAt:row.updatedAt||nowISO() };
return db.rows[row.id];
}

/** ===========================

- PERSIST (audio.js compat)
- =========================== */
  ...
  try{ await apiJson(`/items/${encodeURIComponent(it.id)}`, { method:“PATCH”, json:itemToPatchPayload(it) }); }catch{}
  }
...
- =========================== */
  ...
  else{ addMenu.style.display = “block”; addMenu.setAttribute(“aria-hidden”,“false”); }
  }
...
closeAddMenu();
ensureRefreshBtn();
if(refreshBtn){ refreshBtn.style.display = “”; refreshBtn.title = “Обновить”; refreshBtn.setAttribute(“aria-label”,“Обновить”); }
try{
...
closeAddMenu();
ensureRefreshBtn();
if(refreshBtn){ refreshBtn.style.display = “”; refreshBtn.title = “Обновить ряд”; refreshBtn.setAttribute(“aria-label”,“Обновить ряд”); }
try{
const parent = addMenuBtn && addMenuBtn.parentElement;
...
if(viewMode === “puchok”){
const p = getPuchokLocal(currentPuchokId);
if(!p){ viewMode=“list”; currentPuchokId=null; render(); return; }
...
if(!p || !row){ viewMode=“puchok”; currentRowId=null; render(); return; }
...
const items = […(cached.items || [])].sort((a,b)=>(a.updatedAt||a.createdAt||””).localeCompare(b.updatedAt||b.createdAt||””));
...
```
// Кнопка "Добавить первый элемент" прямо в пустом состоянии
const addBtn = document.createElement("button");
addBtn.className = "btnGhost";
...
// Нормализуем индекс карусели
currentCarouselIndex = clamp(currentCarouselIndex, 0, items.length - 1);
const it = items[currentCarouselIndex];
const total = items.length;
const idx = currentCarouselIndex;

// ── Карусель-контейнер ──────────────────────────────────────
const carousel = document.createElement(“div”);
carousel.className = “carousel”;
carousel.style.cssText = `display:flex; flex-direction:column; gap:0; background:#fff; border-radius:16px; box-shadow:0 2px 16px rgba(0,0,0,0.08); overflow:hidden; margin:12px 0;`;

// ── Верхняя полоса: счётчик + навигация ─────────────────────
const nav = document.createElement(“div”);
nav.style.cssText = `display:flex; align-items:center; justify-content:space-between; padding:10px 14px; border-bottom:1px solid #f0f0f0;`;

const navLeft = document.createElement(“button”);
navLeft.textContent = “←”;
navLeft.style.cssText = `font-size:20px; background:none; border:none; cursor:pointer; color:${idx > 0 ? "#111317" : "#ccc"}; padding:4px 8px;`;
...
counter.style.cssText = `font-size:13px; font-weight:600; color:#555;`;
counter.textContent = `${idx + 1} из ${total}`;

const navRight = document.createElement(“button”);
navRight.textContent = “→”;
navRight.style.cssText = `font-size:20px; background:none; border:none; cursor:pointer; color:${idx < total - 1 ? "#111317" : "#ccc"}; padding:4px 8px;`;
navRight.disabled = idx === total - 1;
navRight.addEventListener(“click”, ()=>{
currentCarouselIndex = Math.min(total - 1, currentCarouselIndex + 1);
render();
});
...
// ── Превью текущего элемента ─────────────────────────────────
const preview = document.createElement(“div”);
preview.style.cssText = `padding:16px; min-height:120px; cursor:pointer;`;
preview.addEventListener(“click”, ()=> openItemFromRow(row.id, it.id));

const previewTitle = document.createElement(“div”);
previewTitle.style.cssText = `font-weight:700; font-size:15px; margin-bottom:6px; color:#111317;`;
previewTitle.textContent = it.title || “Элемент”;

const previewBody = document.createElement(“div”);
previewBody.style.cssText = `font-size:13px; color:#666; line-height:1.5;`;

if(it.type === “text”){
const p2 = (it.content || “”).trim().replace(/\s+/g,” “);
previewBody.textContent = p2 ? (p2.length > 120 ? p2.slice(0,120)+”…” : p2) : “Пустой текст”;
}else if(it.type === “code”){
previewBody.style.fontFamily = “monospace”;
previewBody.style.background = “#f5f5f5”;
previewBody.style.padding = “8px”;
previewBody.style.borderRadius = “8px”;
previewBody.style.fontSize = “12px”;
const p2 = (it.content || “”).trim();
previewBody.textContent = p2 ? (p2.length > 120 ? p2.slice(0,120)+”…” : p2) : “Пустой код”;
}else if(it.type === “link”){
previewBody.innerHTML = `🔗 <span style="color:#4a90e2; word-break:break-all;">${escapeHTML(it.url || "—")}</span>`;
}else if(it.type === “image”){
previewBody.textContent = `📷 ${fmtBytes(it.size)} • нажми чтобы открыть`;
}else if(it.type === “file”){
previewBody.textContent = `📎 ${it.mime || "file"} • ${fmtBytes(it.size)} • нажми чтобы открыть`;
}else if(it.type === “audio”){
const segs = (it.segments || []).length;
previewBody.textContent = `🎙 Сегментов: ${segs} • нажми чтобы открыть`;
}else{
previewBody.textContent = fmtDate(it.createdAt || nowISO());
}

// Метка типа
const tl = typeLabel(it);
const typePill = document.createElement(“span”);
typePill.className = tl.cls;
typePill.textContent = tl.text;
typePill.style.cssText = `display:inline-block; margin-top:8px; font-size:11px;`;

preview.appendChild(previewTitle);
preview.appendChild(previewBody);
preview.appendChild(typePill);
...
touchStartX = null;
if(Math.abs(dx) < 40) return; // слишком маленький свайп
if(dx < 0 && currentCarouselIndex < total - 1){
...
// ── Нижняя панель: удалить + добавить ───────────────────────
const actions = document.createElement(“div”);
actions.style.cssText = `display:flex; align-items:center; justify-content:space-between; padding:10px 14px; border-top:1px solid #f0f0f0; gap:8px;`;

const deleteBtn = document.createElement(“button”);
deleteBtn.textContent = “🗑 Удалить”;
deleteBtn.style.cssText = `background:none; border:1px solid #e0e0e0; color:#c0392b; border-radius:10px; padding:8px 14px; font-size:13px; cursor:pointer;`;
...
try{
  ...
  // обновляем индекс чтобы не выйти за пределы
  if(currentCarouselIndex >= total - 1) currentCarouselIndex = Math.max(0, total - 2);

  await loadRowWithItems(row.id);
  render();
}catch(err){
...
const addBtn = document.createElement(“button”);
addBtn.textContent = “+ Добавить”;
addBtn.style.cssText = `background:#111317; color:#fff; border:none; border-radius:10px; padding:8px 14px; font-size:13px; cursor:pointer;`;
...
// ── Точки-индикаторы (если элементов > 1 и ≤ 10) ────────────
if(total > 1 && total <= 10){
const dots = document.createElement(“div”);
dots.style.cssText = `display:flex; justify-content:center; gap:6px; padding:8px 0 12px;`;
for(let i = 0; i < total; i++){
const dot = document.createElement(“div”);
dot.style.cssText = `width:7px; height:7px; border-radius:50%; background:${i === idx ? "#111317" : "#ddd"}; transition:background 0.2s; cursor:pointer;`;
const di = i;
dot.addEventListener(“click”, ()=>{
currentCarouselIndex = di;
render();
});
...
if(isBusy) return;
isBusy = true;
currentCarouselIndex = 0; // сбрасываем карусель при открытии нового ряда
try{
currentRowId = rowId;
viewMode = “row”;
...
try{ for(const [rid, pack] of Object.entries(db.rows || {})){ if(pack?.row?.puchokId === p.id) delete db.rows[rid]; } }catch{}
viewMode = “list”;
currentPuchokId = null;
currentRowId = null;
}catch(e){
...
- =========================== */
  ...
  const p = getPuchokLocal(currentPuchokId);
  if(!p){ alert(“Сначала открой пучок.”); return null; }
  return p;
  }
...
isBusy = true;
try{
await apiJson(`/puchki/${encodeURIComponent(p.id)}/subpuchok`, { method:“POST”, json:{ title } });
...
const data = await apiJson(`/puchki/${encodeURIComponent(puchokId)}/rows`, { method:“POST”, json:{ type, title } });
...
const data = await apiJson(`/rows/${encodeURIComponent(rowId)}/items`, { method:“POST”, json:payload });
...
- =========================== */
  ...
  await loadRowWithItems(rowId);
  // переходим к новому элементу в карусели
  const pack = db.rows[rowId];
  if(pack){
  const sorted = […(pack.items||[])].sort((a,b)=>(a.updatedAt||a.createdAt||””).localeCompare(b.updatedAt||b.createdAt||””));
  const newIdx = sorted.findIndex(x => x.id === created.id);
  currentCarouselIndex = newIdx >= 0 ? newIdx : sorted.length - 1;
  }
  await openRow(rowId);
...
const p = ensureCurrentPuchok();
if(!p) return;
const lines = (rawInput || “”).toString().split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
if(lines.length === 0){ alert(“Вставь ссылку.”); return; }
isBusy = true;
try{
let rowId;
if(viewMode === “row” && currentRowId){
...
if(!u) continue;
await createItemInRow(rowId, { type:“link”, title:urlTitle(u), url:u });
}
await loadRowWithItems(rowId);
currentCarouselIndex = (db.rows[rowId]?.items||[]).length - 1;
await openRow(rowId);
}catch(e){
...
const created = await createItemInRow(rowId, { type:“file”, title, mime, size, meta:{ r2:{ hasBlob:false, name:title, mime } } });
...
await apiJson(`/items/${encodeURIComponent(it.id)}`, { method:“PATCH”, json:itemToPatchPayload(it) });
await loadRowWithItems(rowId);
currentCarouselIndex = (db.rows[rowId]?.items||[]).length - 1;
await openRow(rowId);
}catch(e){
...
if(pLocal){ pLocal.items = pLocal.items || []; pLocal.items.unshift(it); pLocal.updatedAt = it.updatedAt||nowISO(); }
await loadRowWithItems(rowId);
currentCarouselIndex = 0;
await openRow(rowId);
return it;
...
const url = it.url || “”;
modalViewer.innerHTML = `<div class="fileRow"> <div class="fileMeta"> <div class="fileName">${escapeHTML(it.title || "Ссылка")}</div> <div class="fileSub" style="word-break:break-all">${escapeHTML(url)}</div> </div> <div class="tagText tagLink">Ссылка</div> </div> <div class="viewerActions"> <button class="btnGhost" id="btnOpenLink" ${url?"":"disabled"}>Открыть</button> <button class="btnGhost" id="btnCopyLink" ${url?"":"disabled"}>Copy</button> </div>`;
...
modalHint.textContent = `${label}: метаданные в D1, blob в R2.`;
modalViewer.innerHTML = `<div class="empty">Загружаю файл из облака…</div>`;
let blob = null;
try{ blob = await downloadItemBlobFromR2(it.id); }
catch(e){ modalViewer.innerHTML = `<div class="empty">Ошибка загрузки: ${escapeHTML(e?.message||e)}</div>`; return; }
if(!blob){ modalViewer.innerHTML = `<div class="empty">Blob не найден в R2.</div>`; return; }
const url = URL.createObjectURL(blob);
if(it.type === “image”){
modalViewer.innerHTML = `<img src="${url}" alt="Фото" /> <div class="viewerActions"> <button class="btnGhost" id="btnOpenNewTab">Открыть</button> <button class="btnGhost" id="btnDownload">Скачать</button> </div>`;
}else{
modalViewer.innerHTML = `<div class="fileRow"> <div class="fileMeta"> <div class="fileName">${escapeHTML(it.title||"Файл")}</div> <div class="fileSub">${escapeHTML(it.mime||"file")} • ${fmtBytes(it.size)}</div> </div> <div class="tagText tagFile">Файл</div> </div> <div class="viewerActions"> <button class="btnGhost" id="btnOpenNewTab">Открыть</button> <button class="btnGhost" id="btnDownload">Скачать</button> </div>`;
}
...
if(btnDownload) btnDownload.onclick = ()=>{ const a=document.createElement(“a”); a.href=url; a.download=it.title||(it.type===“image”?“image”:“file”); a.click(); };
return;
}
...
isBusy = true;
try{
await apiJson(`/items/${encodeURIComponent(it.id)}`, { method:“PATCH”, json:{ title:it.title, content:it.content } });
await refreshCurrentRow();
closeModal();
render();
}catch(e){ addMsg(“Ошибка сохранения: “ + (e?.message||e), “err”); }
...
isBusy = true;
try{
if(it.type===“file”||it.type===“image”){ try{ await deleteItemBlobFromR2(it.id); }catch{} }
else{ await cleanupItemBlobsSafe(it); }
await apiJson(`/items/${encodeURIComponent(it.id)}`, { method:“DELETE” });
try{
if(it.type===“audio”){
const p = getPuchokLocal(currentPuchokId);
...
closeModal();
render();
}catch(e){ addMsg(“Ошибка удаления: “ + (e?.message||e), “err”); }
finally{ isBusy = false; }
}
...
- =========================== */
  ...
  if(!currentPuchokId || viewMode === “list”){ alert(“Открой пучок — тогда “В пучок” сохранит ответ туда.”); return; }
...
addMsg(hasToken ? “Чат очищен. Пиши сообщение.” : “Чат очищен. Нет API token — попрошу при отправке.”, “bot”);
}

async function postChatOnce(text){
return await apiFetch(”/chat”, { method:“POST”, json:{ message:text } });
}
...
let data = {};
try{ data = JSON.parse(raw); }catch{}
if(!resp.ok){ addMsg(`HTTP ${resp.status}: ${raw||"error"}`, “err”); }
else if(data && data.ok){ addMsg(data.answer||“Нет ответа”, “bot”); }
else{ addMsg(raw||“Неожиданный ответ”, “err”); }
}catch(e){
addMsg((e&&e.message===“NO_TOKEN”) ? “Нужен API token. Нажми отправить ещё раз.” : (“Ошибка сети: “+(e?.message||e)), “err”);
}
send.disabled = false;
}
...
- =========================== */
  ...
  editPuchokBtn.addEventListener(“click”, ()=>{ closeAddMenu(); renameCurrentPuchok(); });
  addMenuBtn.addEventListener(“click”, (e)=>{ e.stopPropagation(); toggleAddMenu(); });

ensureRefreshBtn();
if(refreshBtn){
refreshBtn.addEventListener(“click”, async (e)=>{ e.stopPropagation(); closeAddMenu(); await refreshStay(); });
}

// — Subpuchok button in menu —
...
closeAddMenu();
if(viewMode === “puchok”) createSubpuchokInCurrent();
});
}

menuAddText.addEventListener(“click”, ()=>{ closeAddMenu(); if(viewMode===“list”){alert(“Сначала открой пучок.”);return;} addTextItemToCurrent(””); });
menuAddFile.addEventListener(“click”, ()=>{ closeAddMenu(); if(viewMode===“list”){alert(“Сначала открой пучок.”);return;} filePicker.value=””; filePicker.click(); });
...
if(!it){ alert(“Не удалось создать голос-элемент.”); return; }
render();
modalWrap.style.display = “flex”;
await openItemFromRow(currentRowId, it.id);
if(typeof startRecordingToAudioItem===“function”) await startRecordingToAudioItem(currentPuchokId, it.id);
else if(typeof createAudioItemAndRecord===“function”) await createAudioItemAndRecord();
else alert(“audio.js не загрузился.”);
}catch(e){ addMsg(“Ошибка голоса: “+(e?.message||e), “err”); }
});
menuAddCode.addEventListener(“click”, ()=>{ closeAddMenu(); if(viewMode===“list”){alert(“Сначала открой пучок.”);return;} addCodeItemToCurrent(””); });
...
clearChatBtn.addEventListener(“dblclick”, ()=>{ promptApiToken(); clearChat(); });
...
- =========================== */
  (async function init(){
  try{ await loadPuchkiList(); }catch(e){}
  viewMode = “list”;
  currentPuchokId = null;
  currentRowId = null;
  currentCarouselIndex = 0;
...
- =========================== */
  ...
  if(t) clearTimeout(t);
  t = setTimeout(()=>{ closeAddMenu(); createSubpuchokInCurrent(); }, 650);
  };
