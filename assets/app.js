...
async function createSubpuchokInCurrent(){
  try{
    ...
    if(!currentPuchokId){
      alert("Нет текущего пучка");
      return;
    }

    await apiFetch(`/puchki/${encodeURIComponent(currentPuchokId)}/subpuchok`,{
      method:"POST"
    });
    ...
    console.error(e);
    alert("Ошибка создания подпучка");
  }
}
...
async function exportPhotoRowZIP(row){
  try{
    ...
    const items = (row.items || []).filter(it => it && it.type === "image");
    if(!items.length){
      alert("В ряду нет фото");
      return;
    }
    ...
    function safeTitle(t){
      if(!t) return "photo";
      return String(t).trim().replace(/[\\/:*?"<>|]/g,"").replace(/\s+/g,"_").slice(0,60) || "photo";
    }
    ...
    for(let i=0;i<items.length;i++){
      ...
      let blob = null;
      try{
        blob = await downloadItemBlobFromR2(it);
      }catch(e){
      ...
      const ext = extFromMime(blob.type || it.mime || "");
      const title = safeTitle(it.title || "photo");
      const idx = String(i+1).padStart(3,"0");
      const filename = `${idx}_${title}${ext}`;
    ...
    const rowTitle = (row.title || "photo_row").replace(/[\\/:*?"<>|]/g,"").trim() || "photo_row";

    saveAs(content, `${rowTitle}.zip`);
...
async function exportPhotoRowHTML(row){
  try{
    ...
    const rowTitle = safeTitle(row.title || "Фото ряд") || "Фото ряд";
...
<div class="caption">${p.title}</div>
    ...
    const safeRow = rowTitle.replace(/\s+/g,"_");

    saveAs(blob, `${safeRow}.html`);

  }catch(e){
    console.error(e);
    alert("Ошибка экспорта HTML");
  }
}
...
async function deleteSubpuchok(subId){
  ...
  const ok = confirm("Удалить подпучок?");
  if(!ok) return;

  try{

    await apiFetch(`/puchki/${encodeURIComponent(subId)}`,{
      method:"DELETE"
    });
    ...
    console.error(e);
    alert("Ошибка удаления подпучка");
  }
}
